...
import { useChallengeDate } from "../../hooks/useDate";
...
const ChallengeScreen = ({route, navigation}) => {
    ...
    const buildPeriodString = (periode) => {
        const debut = new Date(periode.debut);
        const fin = new Date(periode.fin);
        console.log(fin);
        return debut.getDate() + " " + monthNames[debut.getMonth()] + " - " + fin.getDate() + " " + monthNames[fin.getMonth()];
    }

    const isEventOpened = (periode) => {
    ...
    return (
        <ScrollView>
            ...
            <StatusBar hidden />
            <Cover>
                <Image source={challengeData.image} />
                <Title>{challengeData.title}</Title>
                <Subtitle>xx participants</Subtitle>
                <Caption>{useChallengeDate(challengeData?.periode.debut, challengeData?.periode.fin)}</Caption>
            </Cover>
            ...
            <Content>
            {isEventOpened(challengeData?.periode) &&
            <>
            <ContentHeading>
                <ContentHeadingText>Ce challenge vous tente ?</ContentHeadingText>
            </ContentHeading>
            ...
            <ContentLine>
                ...
                <ContentLineText>
                    ...
                    <ContentLineSubText>{challengeData?.objectifLabel}</ContentLineSubText>
                </ContentLineText>
            </ContentLine>
            <ContentLine>
                <ContentLineIcone>
                ...
                <ContentLineText>
                    <ContentLineHeadText>Période</ContentLineHeadText>
                    <ContentLineSubText>{buildPeriodString(challengeData?.periode)}</ContentLineSubText>
                </ContentLineText>
            </ContentLine>
            <ContentLine>
                <ContentLineIcone>
                ...
                <ContentLineText>
                    ...
                    <ContentLineSubText>{buildChallengeString(challengeData?.type)}</ContentLineSubText>
                </ContentLineText>
            </ContentLine>
            <ContentLine>
                <ContentLineIcone>
            ...
            <ContentDetail>
                ...
                <ContentDetailDescription>{challengeData?.detail}</ContentDetailDescription>
            </ContentDetail>
            </Content>
            </Container>
        </ScrollView>
